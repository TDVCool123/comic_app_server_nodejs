...
class Crawler {
    ...
    async allCategories() {
        const body = await util_1.GET(util_1.BASE_URL + "/");
        const categories = this.getCategories(body);
        const images = await this.fetchImagesIfNeeded(categories.map(c => util_1.BASE_URL + c.link));
    ...
    static async getFirstImage(categoryLink) {
        ...
        const thumbnail = util_1.bodyToComicListNew(body)[0].thumbnail;
